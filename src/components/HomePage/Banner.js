...
const Banner = () => {
  ...
  useEffect(() => {
    const dataNoObs = data.filter((data) => {
      return (
        data.PRECIO_MIN_1 != 1 &&
        data.PRECIO_MIN_1 != null &&
        !isNaN(data.PRECIO_MIN_1)
      );
    });
  ...
  const [currentPage] = useState(
    Math.floor(
      Math.random() *
        (infoBanner.length ? infoBanner.length - 5 : infoBanner.length)
    )
  );
  ...
  const listBannerSlice = currentPosts.map((item) => (
    <BannerInfo key={item.ID_ITEM} item={item} />
  ));
  ...
  return (
    <div className="Banner row col-sm-12 col-md-10  col-xl-6 mx-auto p-0">
      {listBannerSlice}
      <ul className="pageNumbersBanner">{renderPageNumbers}</ul>
    </div>
  );
};
